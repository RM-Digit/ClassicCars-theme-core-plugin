...
class TaxonomySearchField extends SearchField
{
    ...
    const HIDE_UNTIL_PARENT_SELECTED = 'hide_until_parent_selected';
    const DISABLE_UNTIL_PARENT_SELECTED = 'disable_until_parent_selected';
     ...
     * @return bool
     */
    public function disableUntilParentSelected()
    {
        if (!$this->searchable->hasParentTaxonomy()) {
            return false;
        }

        if (!isset($this->config[self::DISABLE_UNTIL_PARENT_SELECTED])) {
            return false;
        }
     ...
     * @return bool
     */
    public function hideUntilParentSelected()
    {
        if (!$this->searchable->hasParentTaxonomy()) {
            return false;
        }

        return !empty($this->config[self::HIDE_UNTIL_PARENT_SELECTED]);
    }
